...
export class Movement {
  ...
  getNextPosition = (x, y, delta) => {
    let { x: nextX, y: nextY } = this._getNextPosition(x, y, delta);
    ...
    if (this.settings.bounce) {
      if (nextX <= 0 || nextX >= 500 - 10) {
        ...
        if (this.settings.accelerateOnBounce) this._accelerationTarget = this.settings.maxAcceleration || 5;
      }
  ...
  accelerate = () => {
    if (this._accelerationTarget < this._acceleration) this._acceleration -= 0.5;
    if (this._accelerationTarget > this._acceleration) this._acceleration += 1;
